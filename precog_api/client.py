...
class PrecogClient:
    ...
    def get_recent_predictions(
        ...
        if asset_type is not None and asset_type not in ["BTC", "ETH", "TAO"]:
            raise ValueError("asset_type must be 'BTC', 'ETH', or 'TAO'")

        params = {"limit": limit}
        if asset_type is not None:
            params["asset_type"] = asset_type

        return self._make_request("/predictions/recent", params)
    
    def get_recent_predictions_by_uid(
        self,
        miner_uid: int,
        limit: int = 100,
        asset_type: Optional[Literal["BTC", "ETH", "TAO"]] = None
        ...
        return self._make_request(f"/predictions/recent/uid/{miner_uid}", params)
    
    def get_recent_predictions_by_hotkey(
        self,
        miner_hotkey: str,
        limit: int = 100,
        asset_type: Optional[Literal["BTC", "ETH", "TAO"]] = None
